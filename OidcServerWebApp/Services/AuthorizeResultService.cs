...
namespace OidcServerWebApp.Services;
...
public class AuthorizeResultService : IAuthorizeResultService
{
    ...
    public TokenResponse GenerateToken(IHttpContextAccessor httpContextAccessor)
    {
        ...
        if (clientCodeChecker.IsOpenId)
        {
            ...
            //id_token = new JwtSecurityToken(oidcServerUrl, request.ClientId, claims, signingCredentials: credentials,
        ...
        //var access_token = new JwtSecurityToken(oidcServerUrl, request.ClientId, claims_at, signingCredentials: credentials_at,
        //    expires: DateTime.UtcNow.AddMinutes(
        //       int.Parse("5")));
        ...
        claims2.Add("iat", (int)DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1)).TotalSeconds);
        claims2.Add("sub", "someSubject");
        claims2.Add("nonce", clientCodeChecker.Nonce);

        var descriptor = new Microsoft.IdentityModel.Tokens.SecurityTokenDescriptor
        {
            Issuer = oidcServerUrl,
            Audience = "WareLogixOidcClientWebApp",
            Claims = claims2,
            IssuedAt = null,
        ...
        // Microsoft.IdentityModel.JsonWebTokens.JsonWebToken
        //Microsoft.IdentityModel.JsonWebTokens.JsonWebToken jsonWebToken = new Microsoft.IdentityModel.JsonWebTokens.JsonWebToken()
        return new TokenResponse
        {
            access_token = handler.CreateToken(descriptor),
            //new JsonWebTokenHandler().CreateToken("some_access_token"),
            //new JwtSecurityTokenHandler().WriteToken(access_token),
            id_token = handler.CreateToken(descriptor),
            //new JsonWebTokenHandler().CreateToken("esome_id_token"),
            //id_token != null ? new JwtSecurityTokenHandler().WriteToken(id_token) : null,
            code = request.Code
        };
    }
